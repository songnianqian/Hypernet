...
## A note on centroid count --the lighthouses are few, the positions are not

The obvious objection to a small M is “128 centroids means only 128 concepts — surely that caps how much the controller can represent.” It does not, and the reason is worth stating carefully, because it is a common misread of how the signal works.
If the controller hard-assigned each token to its nearest centroid, then yes: 128 centroids would be one-of-128, about 7 bits, genuinely limited. But it does not hard-assign. It measures the similarity (distance) to all M centroids at once and keeps the whole profile. That is not a choice among 128 things — it is a point in an M-dimensional similarity space. Think of the centroids as lighthouses in a knowledge ocean: with a handful of lighthouses you don’t get a handful of locations, you get a coordinate system that triangulates an enormous number of positions — the way three satellites locate any point on Earth, not just three points. The lighthouse *count* is the number of reference beacons; the *positions they fix* are effectively unbounded. The centroids are few; the meaning-space they address is vast.
The sin/cos channels add a second dimension to each reading: phase. Encoding the rotated features as sine and cosine records not just *how close* a token sits to each lighthouse but *in what direction* — and phases compose. When phase-encoded signals superpose, aligned phases reinforce (a soft AND) and opposed phases cancel (a soft NOT/XOR), so a single vector can carry several relationships at once and let the network separate them again downstream. This is the same mechanism that gives rotary position encodings and vector-symbolic (holographic) memory their power: phase lets you superpose multiple bindings in one signal and pull them back apart. So the reasoning vector isn’t a 7-bit label — it is a dense, phase-encoded coordinate into a large concept-space, and its information content scales with the *dimensionality and precision of the distance-and-phase profile*, not with the raw count of centroids.
Two honest boundaries, so the claim stays strong rather than overreaching. First, capacity *present* is not capacity *used*: the injected signal is only as expressive as the downstream MLP’s ability to read it and as the training signal that shapes the centroids — which is exactly why the Dyck result matters, since it is the evidence the richness is actually exploited, not merely available. Second, the phase-interference-as-logic story is an analogy to vector-symbolic architectures, not a proof that the network learns clean boolean gates; the honest statement is that phase composition supplies the *substrate* for logical combination, and the ablation is what shows the behavior. A clean way to demonstrate the point empirically would be a concept-space map — showing that centroid-similarity profiles for related inputs sit in sensible geometric relationships — which turns the lighthouse metaphor from an image into a measurement.

## One shared frame across all layers — why sharing is correct, not just cheap

The centroids are k-means-style cluster centers in the residual stream — the single E-dimensional space every layer reads from and writes to. Because the residual stream is a shared bus (identity skip connections preserve a common coordinate system up the stack), centroids fit once in that space stay meaningful at every layer. So one shared hypernet serving all layers isn't a memory compromise; it's correct: every layer's hidden state is a position in the one space the centroids anchor. Sharing is justified by the geometry, not merely tolerated to save weight.

This makes the reasoning vector a trajectory. As a token climbs the layers, its representation moves through the fixed centroid geometry, and the sequence of similarity profiles across depth records where it has been — a within-token history along the depth axis, distinct from attention's history along the sequence axis. A per-layer controller sees a single point; the shared controller, reading the same landmarks at every layer, sees the whole path. The sin/cos rate-of-change channels already in the reasoning vector are the first derivative of that path — which direction the token is moving through concept-space — so the architecture carries the ingredient that would let the controller read not just where a token is but where it's heading.

Seen this way, the hypernet is a shared knowledge substrate: an explicit store every layer consults, rather than knowledge smeared redundantly across each layer's weights. A standard transformer that needs the same structure at twenty layers pays for it roughly twenty times in duplicated weights; a shared store pays once. This is the vertical twin of what domain paths do horizontally — paths remove redundancy across domains, the shared hypernet removes it across depth, and both replace implicit duplication with explicit sharing.

Three kinds of memory should be kept distinct here, because they compose rather than overlap: knowledge (long-term stored structure — what the centroids hold), working memory (transient state within one reasoning chain — the KV cache and the per-path memory bank), and the depth-trajectory (a token's refinement across layers). The hypernet is the first; the memory bank is the second; the trajectory is a property of reading the first through a shared frame. Vertical and horizontal memory in one geometry.

Confidence: the efficiency argument is mechanical — shared-once beats duplicated-per-layer, the same logic behind weight-tying and cross-layer sharing. What is a hypothesis, not a result, is how much of the transformer's per-layer redundancy a shared centroid store can actually absorb; late layers may genuinely need their own version of a concept rather than a shared one. The claim is that the hypernet supplements and shares stored knowledge, not that it replaces it. The measurable test: does adding the shared store let you shrink per-layer weights without losing per-domain quality — and does a token's centroid-similarity profile drift coherently across layers (confirming the shared frame) rather than reshuffling randomly (which would call for per-stage centroids)?
